...
public class TC_013 extends DriverInstance{
	...
	@Test(priority = 0)
	private void TC013() throws InterruptedException {
...
// --------------------------------------------------------------------------------------------------->> Next here
		...
		P3_Products.getLE_ViewProduct().get(3).click();
		action.A_Frame_closeAds();
		assertEquals(driver.getTitle().equals(P3_1_ProductDetail.getTitle()), true);

		P3_1_ProductDetail.getE_QuantityInput().clear();
		P3_1_ProductDetail.getE_QuantityInput().sendKeys("4");
		
		P3_1_ProductDetail.getE_ButtonAddToCard().click();
		P3_1_ProductDetail.getE_ViewCart().click();
		assertEquals(P4_Cart.getLE_Quantity().get(0).getText(), "4");
		}
