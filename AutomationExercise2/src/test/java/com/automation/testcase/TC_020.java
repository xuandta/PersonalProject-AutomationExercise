...
public class TC_020 extends DriverInstance{
	private String keyword = "Tshirt";
...
// --------------------------------------------------------------------------------------------------->> Next here
		...
		assertEquals(driver.getTitle(), P3_Products.getTitle(),"message 02");
		P3_Products.Fullfill_searchProduct(keyword);
		assertEquals(P3_Products.getE_title_text_center().getText().contains("SEARCHED PRODUCTS"), true,"message 03"); 
		for (WebElement i : P3_Products.getLE_ProductName()) {
			assertEquals(i.getText().toLowerCase().replaceAll("[^a-z0-9]", "").contains(keyword.toLowerCase().replaceAll("[^a-z0-9]", "")), true,"message 04");;
		}
		String productName = P3_Products.getLE_ProductName().get(0).getText();
		P3_Products.getLE_AddToCart().get(0).click();
		P3_Products.getE_ViewCart().click();
		assertEquals(P4_Cart.getLE_NameProduct().get(0).getText(), productName);
